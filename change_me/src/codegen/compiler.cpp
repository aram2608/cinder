...
Value* Compiler::Visit(ForStmt& stmt) {
  ...
  // Function* func = Builder->GetInsertBlock()->getParent();
  // BasicBlock* cond_block = BasicBlock::Create(*TheContext, "loop.cond",
  // func); BasicBlock* loop_block = BasicBlock::Create(*TheContext,
  // "loop.body", func); BasicBlock* step_block =
  // BasicBlock::Create(*TheContext, "loop.step", func); BasicBlock* after_block
  // = BasicBlock::Create(*TheContext, "loop.end", func);
...
Value* Compiler::Visit(Variable& expr) {
  ...
  //   return Builder->CreateLoad(loaded_type, symb->alloca_ptr,
  //   expr.name.lexeme);
  // }
